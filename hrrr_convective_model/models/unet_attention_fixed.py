import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np

class ConvBNReLU(nn.Sequential):
    def __init__(self, in_c, out_c, k=3, s=1, p=1):
        super().__init__(
            nn.Conv2d(in_c, out_c, k, s, p, bias=False),
            nn.BatchNorm2d(out_c),
            nn.ReLU(inplace=True)
        )

class AttentionBlock(nn.Module):
    """Attention mechanism for U-Net skip connections"""
    def __init__(self, F_g, F_l, F_int):
        super().__init__()
        # W_g: transform gating signal
        self.W_g = nn.Sequential(
            nn.Conv2d(F_g, F_int, 1, bias=True),
            nn.BatchNorm2d(F_int)
        )
        # W_x: transform skip connection
        self.W_x = nn.Sequential(
            nn.Conv2d(F_l, F_int, 1, bias=True),
            nn.BatchNorm2d(F_int)
        )
        # psi: generate attention coefficients
        self.psi = nn.Sequential(
            nn.Conv2d(F_int, 1, 1, bias=True),
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
        self.relu = nn.ReLU(inplace=True)

    def forward(self, g, x):
        """
        g: gating signal from decoder
        x: skip connection from encoder
        """
        # Resize g to match x's spatial dimensions
        if g.shape[2:] != x.shape[2:]:
            g = F.interpolate(g, size=x.shape[2:], mode='bilinear', align_corners=False)
        
        g1 = self.W_g(g)
        x1 = self.W_x(x)
        psi = self.relu(g1 + x1)
        psi = self.psi(psi)
        return x * psi

class DownBlock(nn.Module):
    """Encoder block with double convolution"""
    def __init__(self, in_c, out_c):
        super().__init__()
        self.conv = nn.Sequential(
            ConvBNReLU(in_c, out_c),
            ConvBNReLU(out_c, out_c)
        )
        self.pool = nn.MaxPool2d(2)
    
    def forward(self, x):
        skip = self.conv(x)
        return self.pool(skip), skip

class UpBlock(nn.Module):
    """Decoder block with attention and double convolution"""
    def __init__(self, in_c, skip_c, out_c):
        super().__init__()
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)
        # After upsampling in_c and concatenating skip_c
        self.conv = nn.Sequential(
            ConvBNReLU(in_c + skip_c, out_c),
            ConvBNReLU(out_c, out_c)
        )
    
    def forward(self, x, skip, attn):
        x = self.up(x)
        
        # Handle size mismatches due to odd dimensions
        if x.shape[2:] != skip.shape[2:]:
            x = F.interpolate(x, size=skip.shape[2:], mode='bilinear', align_corners=False)
        
        # Apply attention to skip connection
        skip = attn(x, skip)  # g=x (upsampled), x=skip
        
        # Concatenate
        x = torch.cat([x, skip], dim=1)
        return self.conv(x)

class TemporalEncoding(nn.Module):
    """Temporal positional encoding for hour-of-day and day-of-year."""
    def __init__(self, d_model=64):
        super().__init__()
        self.d_model = d_model
        # Create learnable projection for time features
        self.time_proj = nn.Linear(4, d_model)  # 4 features: sin/cos for hour and day
        
    def forward(self, x, timestamps):
        """
        x: [B, C, H, W] input features
        timestamps: [B] numpy datetime64 or tensor of hours since epoch
        """
        B, C, H, W = x.shape
        
        # Convert timestamps to hour of day and day of year
        # timestamps expected as hours since some epoch
        hours = timestamps.float()
        
        # Hour of day encoding (0-23 hours)
        hour_of_day = (hours % 24) / 24.0  # Normalize to [0, 1]
        hour_sin = torch.sin(2 * np.pi * hour_of_day)
        hour_cos = torch.cos(2 * np.pi * hour_of_day)
        
        # Day of year encoding (0-365 days)
        day_of_year = ((hours // 24) % 365) / 365.0  # Normalize to [0, 1]
        day_sin = torch.sin(2 * np.pi * day_of_year)
        day_cos = torch.cos(2 * np.pi * day_of_year)
        
        # Stack time features [B, 4]
        time_features = torch.stack([hour_sin, hour_cos, day_sin, day_cos], dim=1)
        
        # Project to d_model dimensions [B, d_model]
        time_embed = self.time_proj(time_features)
        
        # Reshape for broadcasting [B, d_model, 1, 1]
        time_embed = time_embed.unsqueeze(-1).unsqueeze(-1)
        
        # Expand to spatial dimensions [B, d_model, H, W]
        time_embed = time_embed.expand(-1, -1, H, W)
        
        return time_embed


class UNetAttn(nn.Module):
    """U-Net with Attention Gates for weather prediction"""
    def __init__(self, in_ch, out_ch, base_features=64, use_temporal_encoding=True):
        super().__init__()
        nf = base_features
        self.use_temporal_encoding = use_temporal_encoding
        
        # Temporal encoding
        if use_temporal_encoding:
            self.temporal_encoder = TemporalEncoding(nf)
            # Adjust input channels to account for temporal features
            in_ch_adjusted = in_ch + nf
        else:
            in_ch_adjusted = in_ch
        
        # Encoder
        self.inc = nn.Sequential(ConvBNReLU(in_ch_adjusted, nf), ConvBNReLU(nf, nf))
        self.down1 = DownBlock(nf, nf * 2)
        self.down2 = DownBlock(nf * 2, nf * 4)
        self.down3 = DownBlock(nf * 4, nf * 8)
        
        # Bridge
        self.bridge = nn.Sequential(
            ConvBNReLU(nf * 8, nf * 16),
            ConvBNReLU(nf * 16, nf * 16)
        )
        
        # Attention blocks
        self.a3 = AttentionBlock(F_g=nf * 16, F_l=nf * 8, F_int=nf * 4)
        self.a2 = AttentionBlock(F_g=nf * 8, F_l=nf * 4, F_int=nf * 2)
        self.a1 = AttentionBlock(F_g=nf * 4, F_l=nf * 2, F_int=nf)
        self.a0 = AttentionBlock(F_g=nf * 2, F_l=nf, F_int=nf // 2)
        
        # Decoder
        self.up3 = UpBlock(nf * 16, nf * 8, nf * 8)
        self.up2 = UpBlock(nf * 8, nf * 4, nf * 4)
        self.up1 = UpBlock(nf * 4, nf * 2, nf * 2)
        self.up0 = UpBlock(nf * 2, nf, nf)
        
        # Output
        self.outc = nn.Conv2d(nf, out_ch, 1)

    def forward(self, x, timestamps=None):
        # Add temporal encoding if enabled
        if self.use_temporal_encoding and timestamps is not None:
            temporal_features = self.temporal_encoder(x, timestamps)
            x = torch.cat([x, temporal_features], dim=1)
        
        # Encoder
        x0 = self.inc(x)
        x1, skip1 = self.down1(x0)
        x2, skip2 = self.down2(x1)
        x3, skip3 = self.down3(x2)
        
        # Bridge
        bridge = self.bridge(x3)
        
        # Decoder with attention
        up3 = self.up3(bridge, skip3, self.a3)
        up2 = self.up2(up3, skip2, self.a2)
        up1 = self.up1(up2, skip1, self.a1)
        up0 = self.up0(up1, x0, self.a0)
        
        # Output
        return self.outc(up0)

class UNetAttnSmall(UNetAttn):
    """Smaller version for limited GPU memory"""
    def __init__(self, in_ch, out_ch, use_temporal_encoding=True):
        super().__init__(in_ch, out_ch, base_features=32, use_temporal_encoding=use_temporal_encoding)