...
def download_from_aws(date_str, hour, output_dir):
    """Download HRRR from AWS S3"""
    year, month, day = date_str[:4], date_str[4:6], date_str[6:8]
    
    bucket = 'noaa-hrrr-bdp-pds'
    key = f'hrrr.{year}{month}{day}/conus/hrrr.t{hour:02d}z.wrfprsf00.grib2'
...
def download_from_gcs(date_str, hour, output_dir):
    """Download HRRR from Google Cloud Storage"""
    year, month, day = date_str[:4], date_str[4:6], date_str[6:8]
    
    url = f'https://storage.googleapis.com/high-resolution-rapid-refresh/hrrr.{year}{month}{day}/conus/hrrr.t{hour:02d}z.wrfprsf00.grib2'
    output_file = output_dir / f'hrrr.{year}{month}{day}.t{hour:02d}z.wrfprsf00.grib2'
...
def download_hrrr_file(args):
    ...
    date_str, hour, output_dir = args
    
    # Try sources in order
    if download_from_aws(date_str, hour, output_dir):
        return True
    if download_from_gcs(date_str, hour, output_dir):
        return True
...
def main():
    ...
    parser.add_argument('--download-forecasts', action='store_true', help='Also download forecast files f01-f18')
    ...
    while current_date <= end_date:
        ...
        for hour in args.hours:
            tasks.append((date_str, hour, args.output_dir))
        current_date += timedelta(days=1)
    
    print(f"Downloading {len(tasks)} HRRR files...")
    
    # Download in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.parallel) as executor:
        list(tqdm(executor.map(download_hrrr_file, tasks), total=len(tasks)))
    
    # Download forecasts if requested
    if args.download_forecasts:
        print("Downloading forecast files...")
        current_date = start_date
        while current_date <= end_date:
            date_str = current_date.strftime('%Y%m%d')
            for hour in args.hours:
                download_forecast_files(date_str, hour, args.output_dir)
            current_date += timedelta(days=1)
